...
if __name__ == '__main__':
    ...
    infer_loader, infer_set = get_infer_dataloader(args)
    ...
    with open(os.path.join(args.infer_data_dir, 'embeddings.ark'), 'w') as fark, open(os.path.join(args.infer_data_dir, 'utt2spk'), 'w') as futt2spk:
        for i, batch in enumerate(infer_loader):
            ...
            spk = infer_set.data.utt2spk[name]
            for timing, emb in zip(timings, speaker_means):
                start, end, _ = timing
                kaldi_io.write_vec_flt(fark, emb, key=f'{name}-{start:05d}-{end:05d}')
            os.makedirs(ex)
            1/0
